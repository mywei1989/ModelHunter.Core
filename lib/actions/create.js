...
exports.init = function(){
  ...
  createPromise.then(function (trgFiles){
    ...
    Promise.all(readTRGPromiseList).then(function(){
      console.log('Promise.all readTRGPromiseList');
      var csvPromiseList = [];
      /*collectionObjList.forEach(function(collectionObj){
        var csvPromise = csv.read(collectionObj.csvPath);
        csvPromise.then(function(fileInfoArray){
          collectionObj.fileInfo = fileInfoArray;
        });
        csvPromiseList.push(csvPromise);
      });

      Promise.all(csvPromiseList).then(function(){
        console.log(collectionObjList[0]);
      });*/

      console.log(collectionObjList.length);
  ...
  /*createPromise.then(function(trgFiles){
    ...
    console.log(iniPromiseArray.length)
    Promise.all(iniPromiseArray).then(function(collectionObjListAll){
          console.log('Promise.all');
          console.log(collectionObjListAll[0]);
    });
  }).catch(function(err){
    console.log(err);
  });*/
